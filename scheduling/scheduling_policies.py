...
class Process():
    def __init__(self, p_id:int, arrival_time:float, deadline:float, burst_time:float) -> None:
        self.p_id:int = p_id
...
def greedy_scheduling_IPRTT(processes:List[Process]):
    ...
    while(len(unscheduled_jobs)>0):
        ...
        processes_with_min_prtt:List[Process] = [p for p in unscheduled_jobs if PRTT(p,current_time) <= min_prtt]
        min_release_time_process:Process = min(processes_with_min_prtt, key= lambda p: p.arrival_time)
        earliest_beginning_time:float = earliest_beginning(min_release_time_process, current_time)
        while(True):
            #Step 3:
            unscheduled_pool : List[Process] = [ p for p in unscheduled_jobs if p.p_id != min_release_time_process.p_id and (earliest_beginning(p, current_time)+p.burst_time) <= earliest_beginning_time]

            if(len(unscheduled_pool) == 0):
                #Step 5:
                scheduled_jobs.append(min_release_time_process)
                unscheduled_jobs.remove(min_release_time_process)
                current_time = earliest_beginning_time + min_release_time_process.burst_time
                break
            else:
                #Step 4:
                min_earliest_beginning: float = min(unscheduled_pool, key=lambda p: earliest_beginning(p, current_time))
                processes_with_earliest_beginning: List[Process] =  [p for p in unscheduled_pool if earliest_beginning(p, current_time) <= min_earliest_beginning]
                min_prtt_process:Process = min(processes_with_earliest_beginning, key= lambda p: PRTT(p, current_time))
...
def greedy_scheduling_NDPRTT(processes: List[Process]):
    ...
    while(len(unscheduled_jobs)> 0):
    #Step 2:
        min_earliest_beginning:float = min(unscheduled_jobs, key=lambda p: earliest_beginning(p, current_time))
        unscheduled_pool:List[Process] = [p for p in unscheduled_jobs if p.arrival_time <= min_earliest_beginning]
        min_prtt:float = min(unscheduled_pool, key = lambda p: PRTT(p, current_time))
        reduced_unscheduled_pool: List[Process] = [p for p in unscheduled_pool if PRTT(p, current_time)<= min_prtt]
        min_burst_process:Process = min(reduced_unscheduled_pool, key= lambda p: p.burst_time)

        #Step 3:
        scheduled_jobs.append(min_burst_process)
        unscheduled_jobs.remove(min_burst_process)
        current_time = earliest_beginning(min_burst_process, current_time) + min_burst_process.burst_time
