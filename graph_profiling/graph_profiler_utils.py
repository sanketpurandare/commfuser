...
class NodeInfo:
    ...
    5) peak_mem (int): the peak memory usage of the node during execution in
                        bytes.
    6) active_mem (int): the number of bytes active subsequent to the node's
                        execution.
    7) in_peak_interval (bool): flags is True if the node lies in the peak
                                memory interval (when active memory exceeds
                                device memory limit)
    8) total_peak_mem (int): This is the peak memory consumption calculated by
                                simulating the memory usage for low memory mode.
    9) first_forward_access (Node): Reference to the node that first uses self
                                    in the forward pass. Generally populated for
                                    the parameter nodes of the model.
    10) last_forward_access (Node): Reference to the node that last uses self in
                                    the forward pass. Generally populated for
                                    the intermediate (activation) nodes.
    11) first_back_access (Node): Reference to the node that first uses self in
                                    the backward pass. Generally populated for
                                    the intermediate (activation) nodes.
    12) last_back_access (Node): Reference to the node that last uses self in
                                    the backward pass. Generally populated for
                                    the intermediate (activation) nodes.
    """
...
class IntNodeInfo(NodeInfo):
    ...
    for intermediate nodes (activations) in the graph. 1) idle_time (float): The
    idle time is calculated as [(last_backward_acess -
    ...
    def __init__(self):
        ...
        # attributed related to swap, populated during scheduling algorithm
        # (Future Use)
