...
from functorch.compile import draw_graph, aot_function, aot_module
...
from typing import Any, Dict, Iterator, List, OrderedDict, Tuple, Set, Optional
from torch.fx import Interpreter, Node, GraphModule
from torch.profiler import profile, record_function, ProfilerActivity
from enum import Enum,auto
from graph_profiling.graph_profiler_utils import GraphType, NodeInfo, IntNodeInfo, BiDict, M2Model, TensorStatus, get_tensor_stat,  MEM_LIMIT, get_model_graphs, ModelType
...
class CapuchinDistExecutor(Interpreter):
    def __init__(self, graphmod:GraphModule, gtype:GraphType, node_info:Dict[Node,NodeInfo], fw_bw_map:BiDict[Node,Node],\
         recomps:Set[Node], gradients:List[Node], grad_info:Dict[Node, NodeInfo], bucket_list:List[int]):
    ...
    # and any recomputation interpreters
        ...
        if(self.gtype == GraphType.backward):
            for rp in self.recomps:
                rp_info:IntNodeInfo = self.node_info[rp]
                primal_count = len(rp_info.rcomp_primals)
                back_rcomp_srcs:List[Node] = [ self.fw_bw_map[n] for n in rp_info.rcomp_sources]
    ...
    def run_node(self, n:Node) -> Any:
            ...
            if nodes_to_recompute is not None:
                for rnode in nodes_to_recompute:
                    ...
                    with torch.cuda.stream(self.exe_stream):                        
                        rval = r_info.rcomp_executor.run(None,initial_env=r_info.rcomp_executor.env)
        ...
        if(self.gtype == GraphType.backward):
            if (n in self.gradients):
                ...
                if(self.current_bucket is not None and self.bucket_accumulated >= self.current_bucket):
...
class CapuchinDistributed:
    def __init__(self, m_type:ModelType, num_models:int, bucket_size:int,loss_function:F, input_sample:torch.Tensor, target_sample:torch.Tensor) -> None:
        self.input:torch.Tensor = input_sample.cuda()
        self.target:torch.Tensor = target_sample.cuda()
        graph_modules:Dict[str,Any] = get_model_graphs(m_type, num_models, loss_function, self.input, self.target)
    ...
    def profile(self, profile_runs:int):
        ...
        fw_profiler.intermediate_nodes = fw_profiler.intermediate_nodes[self.num_outs:] + \
            fw_profiler.intermediate_nodes[0:self.num_outs]
        fw_profiler.fwd_intermediate_nodes_flags = fw_profiler.fwd_intermediate_nodes_flags[self.num_outs:] + \
            fw_profiler.fwd_intermediate_nodes_flags[0:self.num_outs]
        bw_profiler = M2Profiler(self.bw_module, True, GraphType.backward, fw_profiler.fwd_intermediate_nodes_flags, 
        fw_profiler.intermediate_nodes, fw_profiler.node_info)
        with torch.no_grad():
            fw_outputs = fw_profiler.run(*self.params,*self.buffers, self.input, self.target)
    ...
    def init_executors(self):
        ...
        self.fw_executor = CapuchinDistExecutor(self.fw_module, GraphType.forward, self.node_info, self.fw_bw_map, self.recomps, self.gradients, self.grad_info, self.bucket_list)
        self.bw_executor = CapuchinDistExecutor(self.bw_module, GraphType.backward, self.node_info, self.fw_bw_map, self.recomps, self.gradients, self.grad_info, self.bucket_list)
    ...
    def execute(self, input:torch.Tensor, target:torch.tensor)->torch.Tensor:
        ...
        with torch.no_grad():
            ...
            self.fw_executor.init_env(*self.params, *self.buffers,self.input, self.target)
...
# Functions for Recompute
    ...
    def update_existing_recomps(self, t:Node)->int:
        ...
        for rp in self.recomps:
            ...
            if t in rp_info.rcomp_sources:
                ...
                rp_info.rcomp_sources = [src for src in rp_info.rcomp_sources if src != t]
                rp_info.rcomp_sources.extend(t_info.rcomp_sources)
                rp_info.rcomp_primals.extend(t_info.rcomp_primals)
                exe_count += 1
        return exe_count

    def update_rem_candidates(self, t:Node, exe_count:int, candidates:List[Node])->None:
        t_info:IntNodeInfo = self.node_info[t]
        for cand in candidates:
            ...
            if t in cand_info.rcomp_sources:
                ...
                cand_info.rcomp_sources = [src for src in cand_info.rcomp_sources if src != t]
    ...
    def prep_recomps(self):
        ...
        for rp in self.recomps:
            ...
            rcomp_graph = _extract_graph_with_inputs_outputs(self.fw_module.graph, \
                rp_info.rcomp_primals+rp_info.rcomp_sources, [rp])
            # print("Recomputation Graph for: ", str(rp))
            # print(rcomp_graph)
            rp_info.rcomp_graph_mod = GraphModule(self.fw_module, rcomp_graph)
            rp_info.rcomp_executor = Interpreter(rp_info.rcomp_graph_mod, garbage_collect_values=True)
    ...
    def initMSPS(self, candidates:List[Node], placeholders:List[Node]):
        def populate_sources_from_candidates(node:Node, candidates:List[Node], placeholders:List[Node])\
            ...
            for i_node in inp_nodes:
                if i_node in candidates:
                    ...
                    s , p , r = populate_sources_from_candidates(i_node, candidates, placeholders)
                    srcs.extend(s)
                    primals.extend(p)
        ...
        for cand in candidates:
            ...
            n_info.rcomp_sources, n_info.rcomp_primals, n_info.rcomp_extra = populate_sources_from_candidates(cand, candidates, placeholders)
            r_time:float = 0
